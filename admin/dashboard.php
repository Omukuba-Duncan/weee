    ...
    elseif ($action === 'add_review') {
        ...
        if (!empty($_FILES['photo_file']['name']) && $_FILES['photo_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, $allowed) && $f['size'] <= 2 * 1024 * 1024) {
                ...
                $filename = uniqid('revimg_') . '.' . $ext;
                move_uploaded_file($f['tmp_name'], $destDir . $filename);
                $photo = 'uploads/reviews/' . $filename;
            }
        }

        if ($client_name && $review_text) {
            $stmt = $pdo->prepare("INSERT INTO client_reviews (id, client_name, company, photo, review_text, rating) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([uniqid('rev_'), $client_name, $company, $photo, $review_text, $rating]);
            $msg = "Client review added successfully!";
        } else {
            $error = "Please provide client name and review text.";
        }
        ...
        // Handle uploaded file to replace photo
        if (!empty($_FILES['photo_file']['name']) && $_FILES['photo_file']['error'] === UPLOAD_ERR_OK) {
            $f = $_FILES['photo_file'];
            $ext = strtolower(pathinfo($f['name'], PATHINFO_EXTENSION));
            $allowed = ['jpg','jpeg','png','gif'];
        ...
        if ($id && $client_name) {
            $stmt = $pdo->prepare("UPDATE client_reviews SET client_name = ?, company = ?, photo = ?, review_text = ?, rating = ? WHERE id = ?");
            $stmt->execute([$client_name, $company, $photo, $review_text, $rating, $id]);
            $msg = "Client review updated successfully!";
        }
    } elseif ($action === 'delete_review') {
        $id = $_POST['id'] ?? '';
        $stmt = $pdo->prepare("DELETE FROM client_reviews WHERE id = ?");
        $stmt->execute([$id]);
        $msg = "Client review deleted.";
    } elseif ($action === 'add_resource') {
        $title = trim($_POST['title'] ?? '');
        $description = trim($_POST['description'] ?? '');
        $type = trim($_POST['type'] ?? 'PDF Document');
        $status = trim($_POST['status'] ?? 'active');
        ...
        if (!empty($_FILES['resource_file']['name']) && $_FILES['resource_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, $allowed) && $file['size'] <= 12 * 1024 * 1024) {
                $destDir = __DIR__ . '/../uploads/resources/';
                if (!is_dir($destDir)) mkdir($destDir, 0755, true);
                ...
                $error = 'Upload must be a PDF or Word document under 12 MB.';
            }
        }
        ...
        if ($title && $description) {
            ...
            $stmt = $pdo->prepare("INSERT INTO download_resources (id, title, description, type, file_name, file_path, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([uniqid('res_'), $title, $description, $type, $fileName, $filePath, $status]);
            $msg = "Resource document added and available for download.";
        } else {
            $error = 'Please provide a resource title and description.';
        }
        ...
        if ($id && $title) {
            $stmt = $pdo->prepare("UPDATE download_resources SET title = ?, description = ?, type = ?, file_name = ?, file_path = ?, status = ? WHERE id = ?");
            $stmt->execute([$title, $description, $type, $fileName, $filePath, $status, $id]);
            $msg = "Resource document updated successfully.";
        } else {
            $error = 'Please provide a resource title.';
        }
    } elseif ($action === 'delete_resource') {
        $id = $_POST['id'] ?? '';
        $stmt = $pdo->prepare("DELETE FROM download_resources WHERE id = ?");
        $stmt->execute([$id]);
        $msg = "Resource document removed.";
    }
}
...
$reviews = safeFetch($pdo, "SELECT * FROM client_reviews ORDER BY created_at DESC");
$resources = safeFetch($pdo, "SELECT * FROM download_resources ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="container mb-5">
        ...
        <div class="card border-0 shadow-sm mb-4 bg-white p-2">
            <ul class="nav nav-pills nav-fill flex-column flex-md-row gap-1" id="adminTabs" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link rounded-pill py-2" id="reviews-tab" data-bs-toggle="pill" data-bs-target="#tab-reviews" type="button" role="tab">
                        <i class="fa-solid fa-star me-1 text-warning"></i> Client Reviews (<?= count($reviews); ?>)
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link rounded-pill py-2" id="resources-tab" data-bs-toggle="pill" data-bs-target="#tab-resources" type="button" role="tab">
                        <i class="fa-solid fa-file-pdf me-1 text-danger"></i> Resource Docs (<?= count($resources); ?>)
                    </button>
                </li>
                <li class="nav-item" role="presentation">
            ...
            <div class="tab-pane fade" id="tab-reviews" role="tabpanel">
                <div class="row g-4">
                    <div class="col-lg-7">
                        <div class="card border-0 shadow-sm p-4 bg-white">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h5 class="fw-bold mb-0"><i class="fa-solid fa-star text-warning me-2"></i>Client Reviews</h5>
                                <span class="badge bg-light text-dark border"><?= count($reviews); ?> Live</span>
                            </div>
                            <div class="table-responsive">
                                <table class="table table-hover align-middle">
                                    <thead class="table-light">
                                        <tr><th>Client</th><th>Company</th><th>Rating</th><th class="text-end">Action</th></tr>
                                    </thead>
                                    <tbody>
                                        <?php if (empty($reviews)): ?>
                                            <tr><td colspan="4" class="text-center py-4 text-muted">No client reviews yet.</td></tr>
                                        <?php else: ?>
                                            <?php foreach ($reviews as $rev): ?>
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center gap-3">
                                                        <img src="<?= h($rev['photo']); ?>" alt="<?= h($rev['client_name']); ?>" style="width:56px;height:56px;border-radius:50%;object-fit:cover;border:3px solid #fff;box-shadow:0 6px 18px rgba(0,0,0,0.08);">
                                                        <div>
                                                            <div class="fw-bold"><?= h($rev['client_name']); ?></div>
                                                            <div class="font-xs text-muted"><?= htmlspecialchars(substr($rev['review_text'], 0, 80)); ?><?= strlen($rev['review_text'])>80? '...':''; ?></div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td><?= h($rev['company']); ?></td>
                                                ...
                                                <td class="text-end">
                                                    <div class="d-flex justify-content-end gap-1">
                                                        <button type="button" class="btn btn-sm btn-outline-primary rounded-circle" data-bs-toggle="modal" data-bs-target="#editRev<?= $rev['id']; ?>" title="Edit Review"><i class="fa-solid fa-pen"></i></button>
                                                        <form method="POST" action="dashboard.php" onsubmit="return confirm('Delete this client review?');">
                                                            <input type="hidden" name="action" value="delete_review">
                                                            <input type="hidden" name="id" value="<?= h($rev['id']); ?>">
                                                            <button type="submit" class="btn btn-sm btn-outline-danger rounded-circle"><i class="fa-solid fa-trash"></i></button>
                                                        </form>
                                                    </div>

                                                    <!-- Edit Review Modal -->
                                                    <div class="modal fade text-start" id="editRev<?= $rev['id']; ?>" tabindex="-1" aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered">
                                                            <div class="modal-content border-0 rounded-4 shadow">
                                                                <div class="modal-header bg-light">
                                                                    <h5 class="modal-title fw-bold"><i class="fa-solid fa-pen text-primary me-2"></i>Edit Client Review</h5>
                                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                                </div>
                                                                <form method="POST" action="dashboard.php" enctype="multipart/form-data">
                                                                    <div class="modal-body p-4">
                                                                        ...
                                                                        <input type="hidden" name="id" value="<?= h($rev['id']); ?>">
                                                                        <div class="mb-3">
                                                                            <label class="form-label font-sm fw-semibold">Client Name *</label>
                                                                            <input type="text" name="client_name" class="form-control rounded-pill px-3 py-2 font-sm" value="<?= h($rev['client_name']); ?>" required>
                                                                        </div>
                                                                        <div class="mb-3">
                                                                            <label class="form-label font-sm fw-semibold">Company</label>
                                                                            <input type="text" name="company" class="form-control rounded-pill px-3 py-2 font-sm" value="<?= h($rev['company']); ?>">
                                                                        </div>
                                                                        <div class="mb-3">
                                                                            <label class="form-label font-sm fw-semibold">Photo URL (or upload)</label>
                                                                            <input type="url" name="photo" class="form-control rounded-pill px-3 py-2 font-sm mb-2" value="<?= h($rev['photo']); ?>">
                                                                            <input type="file" name="photo_file" accept="image/*" class="form-control form-control-sm">
                                                                        </div>
                                                                        <div class="mb-3">
                                                                            <label class="form-label font-sm fw-semibold">Rating (1-5)</label>
                                                                            <input type="number" name="rating" class="form-control rounded-pill px-3 py-2 font-sm" value="<?= intval($rev['rating']); ?>" min="1" max="5">
                                                                        </div>
                                                                        <div class="mb-2">
                                                                            <label class="form-label font-sm fw-semibold">Review Text</label>
                                                                            <textarea name="review_text" class="form-control rounded-3 p-3 font-sm" rows="4"><?= h($rev['review_text']); ?></textarea>
                                                                        </div>
                                                                    </div>
                                                                    <div class="modal-footer bg-light">
                                                                        <button type="button" class="btn btn-outline-secondary rounded-pill px-3 font-sm" data-bs-dismiss="modal">Cancel</button>
                                                                        <button type="submit" class="btn btn-primary rounded-pill px-4 font-sm fw-bold"><i class="fa-solid fa-check me-1"></i> Save</button>
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
                    ...
                    <div class="col-lg-5">
                        <div class="card border-0 shadow-sm p-4 bg-white sticky-top" style="top: 90px;">
                            <h5 class="fw-bold mb-3"><i class="fa-solid fa-plus-circle text-primary me-2"></i>Add New Client Review</h5>
                            <form method="POST" action="dashboard.php" enctype="multipart/form-data">
                                <input type="hidden" name="action" value="add_review">
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Client Name *</label>
                                    <input type="text" name="client_name" class="form-control rounded-pill px-3 py-2 font-sm" placeholder="e.g., John Doe" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Company</label>
                                    <input type="text" name="company" class="form-control rounded-pill px-3 py-2 font-sm" placeholder="e.g., Acme Ltd">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Photo URL (or upload)</label>
                                    <input type="url" name="photo" class="form-control rounded-pill px-3 py-2 font-sm mb-2" value="https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=400&q=80">
                                    <input type="file" name="photo_file" accept="image/*" class="form-control form-control-sm">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Rating (1-5)</label>
                                    <input type="number" name="rating" class="form-control rounded-pill px-3 py-2 font-sm" value="5" min="1" max="5">
                                </div>
                                <div class="mb-4">
                                    <label class="form-label font-sm fw-semibold">Review Text *</label>
                                    <textarea name="review_text" class="form-control rounded-3 p-3 font-sm" rows="4" placeholder="Short testimonial..." required></textarea>
                                </div>
                                <button type="submit" class="btn btn-primary fw-bold w-100 rounded-pill py-2"><i class="fa-solid fa-upload me-1"></i> Publish Review</button>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
            <!-- TAB: RESOURCE DOWNLOADS MANAGEMENT -->
            <div class="tab-pane fade" id="tab-resources" role="tabpanel">
                <div class="row g-4">
                    <div class="col-lg-7">
                        <div class="card border-0 shadow-sm p-4 bg-white">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h5 class="fw-bold mb-0"><i class="fa-solid fa-file-pdf text-danger me-2"></i>Resource Documents</h5>
                                <span class="badge bg-light text-dark border"><?= count($resources); ?> Live</span>
                            </div>
                            <div class="table-responsive">
                                <table class="table table-hover align-middle">
                                    <thead class="table-light">
                                        <tr><th>Title</th><th>Type</th><th>Status</th><th class="text-end">Action</th></tr>
                                    </thead>
                                    <tbody>
                                        <?php if (empty($resources)): ?>
                                            <tr><td colspan="4" class="text-center py-4 text-muted">No resources created yet.</td></tr>
                                        <?php else: ?>
                                            <?php foreach ($resources as $res): ?>
                                            <tr>
                                                <td>
                                                    <div class="fw-bold text-dark"><?= h($res['title']); ?></div>
                                                    <div class="font-xs text-muted"><?= h($res['file_name']); ?></div>
                                                </td>
                                                <td><?= h($res['type']); ?></td>
                                                <td>
                                                    <span class="badge <?= $res['status'] === 'active' ? 'bg-success-subtle text-success' : 'bg-secondary-subtle text-secondary'; ?> border">
                                                        <?= h(ucfirst($res['status'])); ?>
                                                    </span>
                                                </td>
                                                <td class="text-end">
                                                    <div class="d-flex justify-content-end gap-1">
                                                        <button type="button" class="btn btn-sm btn-outline-primary rounded-circle" data-bs-toggle="modal" data-bs-target="#editRes<?= h($res['id']); ?>" title="Edit Resource"><i class="fa-solid fa-pen"></i></button>
                                                        <form method="POST" action="dashboard.php" onsubmit="return confirm('Remove this resource document?');">
                                                            <input type="hidden" name="action" value="delete_resource">
                                                            <input type="hidden" name="id" value="<?= h($res['id']); ?>">
                                                            <button type="submit" class="btn btn-sm btn-outline-danger rounded-circle"><i class="fa-solid fa-trash"></i></button>
                                                        </form>
                                                    </div>
                                                </td>
                                            ...
                                            <!-- Edit Resource Modal -->
                                            <div class="modal fade text-start" id="editRes<?= h($res['id']); ?>" tabindex="-1" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content border-0 rounded-4 shadow">
                                                        <div class="modal-header bg-light">
                                                            <h5 class="modal-title fw-bold"><i class="fa-solid fa-pen text-danger me-2"></i>Edit Resource</h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                        </div>
                                                        <form method="POST" action="dashboard.php" enctype="multipart/form-data">
                                                            <div class="modal-body p-4">
                                                                ...
                                                                <input type="hidden" name="id" value="<?= h($res['id']); ?>">
                                                                <input type="hidden" name="existing_file_path" value="<?= h($res['file_path']); ?>">
                                                                <div class="mb-3">
                                                                    <label class="form-label font-sm fw-semibold">Title *</label>
                                                                    <input type="text" name="title" class="form-control rounded-pill px-3 py-2 font-sm" value="<?= h($res['title']); ?>" required>
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label class="form-label font-sm fw-semibold">Type</label>
                                                                    <input type="text" name="type" class="form-control rounded-pill px-3 py-2 font-sm" value="<?= h($res['type']); ?>">
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label class="form-label font-sm fw-semibold">File Name</label>
                                                                    <input type="text" name="file_name" class="form-control rounded-pill px-3 py-2 font-sm" value="<?= h($res['file_name']); ?>">
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label class="form-label font-sm fw-semibold">Upload New PDF</label>
                                                                    <input type="file" name="resource_file" accept=".pdf,.doc,.docx" class="form-control form-control-sm">
                                                                    <?php if ($res['file_path']): ?>
                                                                    <div class="font-xs text-muted mt-2">Current file: <a href="../<?= h($res['file_path']); ?>" target="_blank" class="text-decoration-none"><?= h(basename($res['file_path'])); ?></a></div>
                                                                    <?php endif; ?>
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label class="form-label font-sm fw-semibold">Status</label>
                                                                    <select name="status" class="form-select rounded-pill font-sm">
                                                                ...
                                                                <div class="mb-3">
                                                                    <label class="form-label font-sm fw-semibold">Description</label>
                                                                    <textarea name="description" class="form-control rounded-3 p-3 font-sm" rows="4"><?= h($res['description']); ?></textarea>
                                                                </div>
                                                            </div>
                                                            <div class="modal-footer bg-light">
                                                                <button type="button" class="btn btn-outline-secondary rounded-pill px-3 font-sm" data-bs-dismiss="modal">Cancel</button>
                                                                <button type="submit" class="btn btn-danger rounded-pill px-4 font-sm fw-bold"><i class="fa-solid fa-check me-1"></i> Save Resource</button>
                                                            </div>
                                                        </form>
                                                    </div>
                                                </div>
                    ...
                    <div class="col-lg-5">
                        <div class="card border-0 shadow-sm p-4 bg-white sticky-top" style="top: 90px;">
                            <h5 class="fw-bold mb-3"><i class="fa-solid fa-plus-circle text-danger me-2"></i>Add Resource Document</h5>
                            <form method="POST" action="dashboard.php" enctype="multipart/form-data">
                                <input type="hidden" name="action" value="add_resource">
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Resource Title *</label>
                                    <input type="text" name="title" class="form-control rounded-pill px-3 py-2 font-sm" placeholder="e.g., EPR Compliance Guide" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Type</label>
                                    <input type="text" name="type" class="form-control rounded-pill px-3 py-2 font-sm" value="PDF Document">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">File Name</label>
                                    <input type="text" name="file_name" class="form-control rounded-pill px-3 py-2 font-sm" placeholder="Leave blank to use title.pdf">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Upload PDF / DOC</label>
                                    <input type="file" name="resource_file" accept=".pdf,.doc,.docx" class="form-control form-control-sm">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Status</label>
                                ...
                                <div class="mb-3">
                                    <label class="form-label font-sm fw-semibold">Description *</label>
                                    <textarea name="description" class="form-control rounded-3 p-3 font-sm" rows="4" placeholder="Short resource summary" required></textarea>
                                </div>
                                <button type="submit" class="btn btn-danger fw-bold w-100 rounded-pill py-2"><i class="fa-solid fa-upload me-1"></i> Publish Resource</button>
                            </form>
                        </div>
                    </div>
                </div>
